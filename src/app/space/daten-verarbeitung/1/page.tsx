...
} from "@/app/space/daten-verarbeitung/instagram-profile";
...
export default function DataProcessing1() {
  ...
  return (
    <div className="h-full relative flex @container">
      <div
        className="h-full w-full @2xl:w-1/2 @2xl:max-w-md flex-shrink-0 mr-10 border-2 rounded-xl shadow overflow-hidden">
        <div className="h-full box-border">
          <InstagramProfile profile={profile} className="w-full"/>
        </div>
      </div>
      <div
        className="flex justify-center items-center flex-col gap-10 absolute bottom-0 right-0 z-40 @2xl:relative @2xl:w-full @2xl:h-full">
        <textarea
        ...
        <div className="flex flex-col justify-center items-center gap-10 h-full">
          {showMessage &&
              <p className="max-w-80 box-content w-full text-center px-8 py-6 bg-white shadow-lg rounded-2xl">
                  ...
                  <Button className="w-full mt-4" onClick={() => setShowMessage(false)}>Alles klar!</Button>
              </p>}
          <Robot
            expression="resting"
            className={clsx(showMessage ? "w-48 h-48" : "w-32 h-32", "hover:scale-110 transition-all cursor-pointer duration-500")}
            onClick={onClick}
          />
        </div>
      </div>
