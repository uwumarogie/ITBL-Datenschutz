...
export default function DataProcessing2() {
  ...
  return <div className="relative h-full w-full flex flex-col items-center justify-start">

    <div className="w-full flex flex-col justify-start items-center relative">
      <Robot expression={states[state].expression}
    ...
    <div className={clsx("flex flex-col gap-4 w-full h-full my-4 overflow-y-auto transition-opacity duration-700 delay-300", state >= 1 ? "opacity-100" : "opacity-0")}>
      {advertisements.map(advertisement => (
        <div className="bg-blue-200 rounded-xl w-full flex flex-shrink-0" onClick={() => onAdvertisementClick(advertisement)}>
          <Image src={advertisement.imageSrc} alt={"Advertisement"} width="100" height="100" className="h-full"/>
          <div className="flex flex-col justify-center pl-6 pr-4">
            <span className="text-lg font-semibold text-blue-950">{advertisement.name}</span>
