        ...
        <>
            {moduleFinished ? (
                <div className='flex flex-col items-center text-center gap-6 md:mt-6'>
                    ...
                    <span className='max-w-[600px]'>Du hast das Modul erfolgreich abgeschlossen</span>
                    ...
                    {!instructionsRead ? (
                        <div className='p-2 flex flex-col items-center gap-4 lg:mt-8 max-w-[700px]'>
                            ...
                            <span className="text-center">
                                Jetzt bist du an der Reihe dien Wissen anzuwenden: Du wirst jetzt eine Reihe von Profilen gezeigt
                                bekommen, die entweder Anzeichen von realen Profilen aufweisen oder aber Hinweise drauf enthalten, dass
                                es sich eher um Phishing obzw. Fake-Profile handelt. Ordne die Profile in eine der beiden Kategorien einordnen.
                            </span>
                            <Button onClick={() => setInstructionsRead(true)} className='max-w-[150px] lg:mt-4'>
                                Starten
                            </Button>
                        </div>
                    ) : (
                        <div className='flex flex-row lg:h-[calc(100vh-210px)] flex-wrap gap-2 lg:gap-0 lg:flex-nowrap'>
                            ...
                            <div className="flex flex-col justify-between items-center w-full max-w-[700px] gap-y-4">
                                <div className="flex flex-col items-center mt-4 sm:mt-16 gap-y-4 max-w-[80%] text-center">
                                    <span className="text-lg md:text-2xl text-blue-background">Wie würdest du {profiles[activeIndex].instagramProfile.username} einschätzen?</span>
                                    <div className="max-w-[300px]">
                                        <Button onClick={() => handleRating(false)} style={wrongAnmiation && profiles[activeIndex].isRealProfile ? "red" : "neutral"} className={clsx("w-full mb-4", wrongAnmiation && profiles[activeIndex].isRealProfile && "animate-shake")}>
                                            eher Fake/Phishing-Profil
                                        </Button>
                                        <Button onClick={() => handleRating(true)} style={wrongAnmiation && !profiles[activeIndex].isRealProfile ? "red" : "neutral"} className={clsx("w-full", wrongAnmiation && !profiles[activeIndex].isRealProfile && "animate-shake")}>
                                            eher reales Profil
                                        </Button>
                                    </div>
                                </div>
...
const profiles: Profile[] = [
    ...
    },
    {
        instagramProfile: {
            ...
            followingCount: 365,
            following: dummyFollowings,
            description: "!!KEIN FAKE ECHTE HANDYS!! 	Registrieren und Gewinnen: http://iphonegewinnen.de",
            posts: [{
                imageSrc: "/insta-profiles/gewinnspiel/1.png",
                caption: "!!KEIN FAKE ECHTE HANDYS!! 	Registrieren und Gewinnen: http://iphonegewinnen.de Probiert euer Glück und gewinnt ein neues Iphone 15 (Link) Welche Farbe wollt ihr gewinnen? !Kein fake!",
                likedBy: "Gefällt 32 Mal"
            },
            {
                imageSrc: "/insta-profiles/gewinnspiel/2.jpg",
                caption: "!!KEIN FAKE ECHTE HANDYS!! 	Registrieren und Gewinnen: http://iphonegewinnen.de Sei du die nächste! Gewinne ein brandneues Iphone 15 (Link)",
                likedBy: "Gefällt 31 Mal"
            },]
        },
        isRealProfile: false,
