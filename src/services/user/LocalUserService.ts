import {User, UserDataAchievement, UserService} from "@/services/user/UserService";
...
export class LocalUserService implements UserService {
  ...
  async setAchievement(achievement: string, unlocked: boolean): Promise<boolean> {
    this.load()
    if(this.data == null) {
      throw new Error("Failed to fetch user data");
    }
    const existingAchievement = this.data.achievements.findIndex(a => a.achievementEnum == achievement)
  ...
  async setHighScore(highScoreEnum: HighScoreType, highScore: number): Promise<boolean> {
    this.load()
