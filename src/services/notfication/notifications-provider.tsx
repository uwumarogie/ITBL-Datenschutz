...
export default function NotificationsProvider({
  ...
  const [achievementMessage, setAchievementMessage] = useState<React.ReactNode | undefined>(undefined);
  ...
  return (
    <>
      ...
      {achievementMessage && (
        ...
        <div className={clsx("fixed inset-0 flex items-center justify-center z-50", fadeOut && "animate-fadeOut")}>
          <div className="shadow-3xl animate-slideIn">
            {achievementMessage}
          </div>
          <Explosion
      ...
      <div className={clsx("animate-blurIn", blurOut && "animate-blurOut")} style={{ filter: achievementMessage && !blurOut ? "blur(6px)" : "" }}>
        {children}
      </div>
    </>
  );
